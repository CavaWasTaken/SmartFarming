...
def get_sensors(device_id, device_name):
...
def get_sensor_id(device_id, device_name, greenhouse_id, plant_id, sensor_name, sensor_type):
    conn = get_db_connection() # get the connection to the database
    cur = conn.cursor() # create a cursor to execute queries
    ...
    def GET(self, *uri, **params):
        if len(uri) == 0:
           raise cherrypy.HTTPError(status=400, message='UNABLE TO MANAGE THIS URL')
            ...
            return get_sensors(params['device_id'], params['device_name'])
        elif uri[0] == 'get_sensor_id':
            return get_sensor_id(params['device_id'], params['device_name'], params['greenhouse_id'], params['plant_id'], params['sensor_name'], params['sensor_type'])
        else:
            raise cherrypy.HTTPError(status=400, message='UNABLE TO MANAGE THIS URL')
