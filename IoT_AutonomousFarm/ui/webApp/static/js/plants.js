    ...
    .then((data) => {
      ...
      plants.forEach((plant) => {
        ...
        card.innerHTML = `
          ...
          <div class="desc mt-3">
            <h1 class="primary-text">${plant.name}</h1>
            <h5 class="text-white">${plant.species}</h5>
          </div>
          <div class="details">
            ${Object.entries(plant.desired_thresholds)
              .map(
                ([key, val]) => `
                <div class="rating">
                  <h6 class="primary-text">${val.min || 0} - ${val.max || 0}</h6>
                  <h6 class="secondary-text">${key}</h6>
                </div>`
              )
              .join("")}
          </div>
