document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("./json/WebApp_config.json") // this path is relative to the HTML file
    ...
    .then((config) => {
      ...
      return fetch(
        `${catalog_url}/get_sensors_area?${queryParams}`,
        {
          method: "GET",
          headers: {
          ...
          modal
            ...
            .addEventListener("submit", (e) => {
              ...
              if (sensor.type === "NPK") {
                ...
                if (
                  updatedNMin < sensor.domain.min || updatedNMax > sensor.domain.max ||
                  updatedPMin < sensor.domain.min || updatedPMax > sensor.domain.max ||
                  updatedKMin < sensor.domain.min || updatedKMax > sensor.domain.max
                ) {
              ...
              }else
              {
                ...
                const updatedMax = parseFloat(document.getElementById(`max-threshold-${sensor.sensor_id}`).value); 
                 ...
                 // Validate the thresholds against the domain
                  if (updatedMin < sensor.domain.min || updatedMax > sensor.domain.max) {
                    alert("Threshold values must be within the allowed domain range.");
                    return; // Stop submission if validation fails
                  }
