...
try:
    ...
    with open("./LightManagement_config.json", "r") as config_fd:
        ...
        telegram_token = config["telegram_token"]  # telegram token for sending alerts
...
def checkSensors():
    ...
    try:
        response = requests.get(f'{catalog_url}/get_sensors', params={'greenhouse_id': greenhouse_id, 'device_name': 'LightManagement'})  # read the list of sensors from the Catalog
