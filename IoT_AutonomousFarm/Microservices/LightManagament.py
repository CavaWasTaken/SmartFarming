...
class LightManagement(MqttSubscriber):
    ...
    def on_message(self, client, userdata, msg):
        with open("../logs/LightManagement.log", "a") as log_file: # print all the messages received on a log file
            ...
            if message["bn"] == "greenhouse_1/sensors/LightIntensity":    # check the topic of the message
            ...
            elif message["bn"] == "greenhouse_1/sensors/DTH22/Temperature":   # check the topic of the message
                temperature = message["v"]
                if temperature < 20:
                    log_file.write(f"Temperature is low: {temperature}\n")
                else:
                    log_file.write(f"Temperature is high: {temperature}\n")
            elif message["bn"] == "greenhouse_1/schedules/lighting":  # check the topic of the message
                log_file.write(f"Received lighting schedule: {message["e"]}\n")

if __name__ == "__main__":
    broker = "mqtt.eclipseprojects.io"  # broker address
    port = 1883
    topics = ["greenhouse_1/sensors/LightIntensity", "greenhouse_1/sensors/DTH22/Temperature", "greenhouse_1/schedules/lighting"]    # nutrient values
